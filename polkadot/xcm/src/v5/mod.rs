...
define_xcm_instructions! {
	transact: {
		/// - `fallback_max_weight`: Used for compatibility with previous versions. Corresponds
		///   to the `require_weight_at_most` parameter in previous versions. If you don't care
		///   about compatibility you can just put `None`. WARNING: If you do, your XCM might
		///   not work with older versions. Make sure to dry-run and validate.
	...
	extra: {
		...
		///
		/// Assets in the holding register are matched using the given list of
		/// `AssetTransferFilter`s, they are then transferred based on their specified transfer
		/// type:
		///
		/// - teleport: burn local assets and append a `ReceiveTeleportedAsset` XCM instruction
		///   to the XCM program to be sent onward to the `destination` location,
		///
		/// - reserve deposit: place assets under the ownership of `destination` within this
		///   consensus system (i.e. its sovereign account), and append a
		///   `ReserveAssetDeposited` XCM instruction to the XCM program to be sent onward to
		///   the `destination` location,
		///
		/// - reserve withdraw: burn local assets and append a `WithdrawAsset` XCM instruction
		///   to the XCM program to be sent onward to the `destination` location,
		///
		/// The onward XCM is then appended a `ClearOrigin` to allow safe execution of any
		/// following custom XCM instructions provided in `remote_xcm`.
		///
		/// The onward XCM also contains either a `PayFees` or `UnpaidExecution` instruction
		/// based on the presence of the `remote_fees` parameter (see below).
		///
		/// Parameters:
		/// - `destination`: The location of the program next hop.
		/// - `remote_fees`: If set to `Some(asset_xfer_filter)`, the single asset matching
		///   `asset_xfer_filter` in the holding register will be transferred first in the
		///   remote XCM program, followed by a `PayFees(fee)`, then rest of transfers follow.
		///   If set to `None`, a `UnpaidExecution` instruction is appended instead.
		/// - `preserve_origin`: Specifies whether the original origin should be preserved or
		///   cleared, using `AliasOrigin` or `ClearOrigin` respectively.
		/// - `assets`: List of asset filters matched against existing assets in holding.
		/// - `remote_xcm`: Custom instructions that will be executed on the `destination`
		///   chain after a `ClearOrigin`.
		///
		/// Safety: No concerns.
		///
		/// Kind: *Command*
		...
		/// Executes inner `xcm` with origin set to the provided `descendant_origin`. Once the
		/// inner `xcm` is executed, the original origin (the one active for this instruction)
		/// is restored.
		///
		/// Parameters:
		/// - `descendant_origin`: The origin that will be used during the execution of the
		///   inner `xcm`. If set to `None`, the inner `xcm` is executed with no origin. If
		///   set to `Some(o)`, the inner `xcm` is executed as if there was a
		///   `DescendOrigin(o)` executed before it, and runs the inner xcm with origin:
		///   `original_origin.append_with(o)`.
		/// - `xcm`: Inner instructions that will be executed with the origin modified according
		///   to `descendant_origin`.
		///
		/// Safety: No concerns.
		///
		/// Kind: *Command*
		...
		/// Parameters:
		///
		/// - `hints`: A bounded vector of `ExecutionHint`, specifying the different hints that
		/// will be activated.
